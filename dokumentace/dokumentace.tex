...
Our task was implement table of symbols as hash table. Hash table contains one type of structure, which is same for functions and local variables, but data, which contains are different.
Item which represents function, contains key, pointer on list of parameters, pointer on list of return types.
type (declared/defined) and pointer on next item in list of items. Item, whch represents local variable, contains key, data type (string,number,integer),
 There are two hash tables pointers in \texttt{parser\_data\_t}
